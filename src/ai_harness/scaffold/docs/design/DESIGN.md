...
> Token / 组件是「材料合格」，质量锚是「组装合格」。**Accept 按此复核**；任何一条不过 = 还没到人闸。
>
> ~~原型送人闸前逐条自评 / Critic 两段式评审~~ **已按引擎 ADR-006 移除**（2026-09-24）：原「观感先于核对」的两段式（先看三档截图记不适感、再逐条对锚）降为 agent 写原型的随手内参（不产截图、不写自查记录送闸）；第一眼质量兜底 = design-lint 机械闸 + 人闸肉眼看预览链接。锚清单本身保留——它是 DESIGN.md 的组装纪律，项目 Accept / 人审仍可对表。
